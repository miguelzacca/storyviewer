document.addEventListener('DOMContentLoaded', () => {
  ...
  function showResults(username, count) {
    ...
    storyCountEl.textContent = count === 0
      ? 'No active stories'
  ...
  function renderLightboxSlide() {
    ...
    lbDownload.href = (story.isVideo && story.videoUrl) ? story.videoUrl : story.url;
  ...
  lbPrev.addEventListener('click', () => {
    if (currentIndex > 0) { currentIndex--; renderLightboxSlide(); }
  });

  lbNext.addEventListener('click', () => {
    if (currentIndex < currentStories.length - 1) { currentIndex++; renderLightboxSlide(); }
  });
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowLeft')  { if (currentIndex > 0) { currentIndex--; renderLightboxSlide(); } }
    if (e.key === 'ArrowRight') { if (currentIndex < currentStories.length - 1) { currentIndex++; renderLightboxSlide(); } }
  });
