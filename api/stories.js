  ...
  try {
    ...
    const _res = await fetch('https://www.instagram.com/graphql/query', {
      headers: {
        ...
        'x-asbd-id': '359341',
        'x-bloks-version-id': '8c6b886e8c8169480f69ddea406ef61a5b9b1e99aa1fb678ce42845892fffdd4',
    ...
    const id = _data?.data?.xdt_api__v1__feed__user_timeline_graphql_connection?.edges?.[0]?.node?.user?.pk;

    if (!id) {
      return res.status(404).json({ error: 'User not found or ID could not be retrieved.' });
    }

    // 2. Fetch Stories
    const res2 = await fetch('https://www.instagram.com/graphql/query', {
      headers: {
        accept: '*/*',
        'accept-language': 'en-US,en;q=0.9,pt;q=0.8',
        'cache-control': 'no-cache',
